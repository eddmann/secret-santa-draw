...
import { clearDrawStatus, drawStatusSelector } from '@/store/localDraws';
import { draw } from '@/store/localDraws/actions';
import { entrySelector, toggleExclusion } from '@/store/localEntry';
...
export const ConductLocalDrawExclusions = () => {
  ...
  useEffect(() => {
    if (drawStatus.type === 'drawn') {
      ...
      navigate(`/local/draws/${drawStatus.id}`, { replace: true });
    }
  ...
  const handleExclusion = (participant: LocalParticipant, exclusion: LocalParticipant) => () => {
    setDrawError('');
  ...
  return (
    <>
      <Header
        ...
        icon={<BackIcon />}
        onClick={() => {
          navigate('/locale/draws/conduct/participants');
        }}
      />
      <Content>
